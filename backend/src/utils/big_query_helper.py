...
class BigQueryHelper:
    ...
    def insert_dict_to_table(
        self, table_name: str, input_dict: dict
    ) -> Optional[Sequence[dict]]:
        """
        dict를 table에 업로드. 이 때, table과 dict는 이름이 모두 동일해야한다.
        ...
        if len(schemas) != len(input_dict):
            raise ValueError(
                f"Table의 column 개수 {len(schemas)}와 dict의 원소 개수 {len(input_dict)}가 달라요."
            )

        errors = self.bigquery_client.insert_rows_json(
            table=table, json_rows=(input_dict,)
        )
        for error in errors:
            if error:
                return error
