from src.AI.models.model_object import (
    MCNTopkDataset,
    MCNProductDataset,
    SimpleMCN,
)
...
class SimpleMCNInference:
    def __init__(
        ...
        if not os.path.exists(self.model_path):
            ...
            gcs_helper.download_file_from_gcs(
                blob_name="SimpleMCN/SimpleMCN_latest.pt", file_name=self.model_path
            )
    ...
    async def diagnosis(self, equips: Dict[str, int]):
        ...
        score = self.model.forward(images)
        return float(score)
